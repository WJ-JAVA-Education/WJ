...
public class PollMgr {
		...
		try {
			...
			if(cnt==1) {//정상적으로 설문리스트 저장 -> 설문아이템 실행
				...
				for (int i = 0; i < item.length; i++) {
					...
					pstmt.setString(3, item[i]);//2020년 상반기
	...
	//select num, question, type, active from tblPollList where num=?
	...
	//Poll View : 투표결과
	//필요한 값은 item이랑 count이다. 두개를 넣을수 있는 객체는 빈즈가 현재 유일
	//그래서 빈즈에 두개의 값을 저장하고 이 빈즈를 Vector add 하도록 하겠다.
