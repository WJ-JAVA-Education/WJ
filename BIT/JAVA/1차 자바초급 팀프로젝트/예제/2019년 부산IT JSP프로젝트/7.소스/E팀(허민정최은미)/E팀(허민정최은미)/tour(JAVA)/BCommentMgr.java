...
public class BCommentMgr {
		...
		public Vector<BCommentBean> getBComment(int num){
			...
			try {
				...
				sql = "select * from toBComment where num=?";
		...
		public void insertBComment(BCommentBean bean){
			...
			try {
				con = pool.getConnection();
				sql = "insert toBComment(name,comment,regdate,num) "
						+ "values(?,?,now(),?)";
				pstmt = con.prepareStatement(sql);
		...
		public int getBCommentCount(int num){
			...
			try {
				con = pool.getConnection();
				sql = "select count(num) from toBComment where num=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, num);
				rs = pstmt.executeQuery();
				if(rs.next())
		...
		public void deleteBComment(int cnum){
			...
			try {
				con = pool.getConnection();
				sql = "delete from toBComment where cnum=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, cnum);
				pstmt.executeUpdate();
			} catch (Exception e) {
		...
		public void deleteAllBComment(int num){
			...
			try {
				con = pool.getConnection();
				sql = "delete from toBComment where num=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, num);
				pstmt.executeUpdate();
			} catch (Exception e) {
